...
struct Node* root;
int duplicate_count = 0; // Переменная для подсчета отказов
...
struct Node* CreateTree(struct Node* root, struct Node* r, int data) // создание дерева
{
    ...
    if (data == r->data) {
        ...
        duplicate_count++; // Увеличиваем счетчик при отказе добавления
        return root;
    }
...
// Инверсии дерева
void invert_tree(struct Node* r) {
    if (r == NULL) return;
...
int main()
{
    ...
    int choice;
    while (true) {
        printf("Для инверсии введите 1, для окончания 0: ");
        scanf_s("%d", &choice);

        if (choice == 1) {
            invert_tree(root); 
            printf("Инвертированное дерево:\n");
            print_tree(root, 0); 
        }
        else if (choice == 0) {
            break; 
